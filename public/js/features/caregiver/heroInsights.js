// Caregiver story metrics — confidence language (browser)
...
const HeroInsights = (() => {
  ...
  function formatLastActive(iso, now = new Date()) {
    if (!iso) return { label: 'No sessions yet', isToday: false };
    const when = new Date(iso);
    const today = startOfLocalDay(now);
    const day = startOfLocalDay(when);
    const diffDays = Math.round((today - day) / MS_DAY);
    const time = when.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
    if (diffDays === 0) return { label: `Today, ${time}`, isToday: true };
    if (diffDays === 1) return { label: `Yesterday, ${time}`, isToday: false };
    if (diffDays < 7) return { label: `${diffDays} days ago`, isToday: false };
    return {
      label: when.toLocaleDateString([], { month: 'short', day: 'numeric' }),
      isToday: false,
    };
  }

  function classifyTrendLocal(sessions) {
    const scored = sessions
      .filter((s) => s.status === 'completed' && s.accuracy != null)
      .sort((a, b) => (a.startedAt || '').localeCompare(b.startedAt || ''));
    if (scored.length < 3) return { state: 'insufficient_data' };
    const recent = scored.slice(-3);
    const prior = scored.slice(-6, -3);
    const avg = (list) => list.reduce((s, x) => s + (x.accuracy || 0), 0) / (list.length || 1);
    if (prior.length < 3) return { state: 'stable' };
    const delta = (avg(recent) - avg(prior)) * 100;
    if (delta <= -20) return { state: 'declining' };
    if (delta >= 15) return { state: 'improving' };
    return { state: 'stable' };
  }
  ...
  function build(patient, rawSessions, opts = {}) {
    ...
    const trendState = opts.trendState || classifyTrendLocal(completed).state;
    ...
    if (weekAvg != null && priorAvg != null) weekDeltaPct = (weekAvg - priorAvg) * 100;
    else if (weekAvg != null && completed.length >= 3) {
    ...
    const overall = overallFromDelta(weekDeltaPct, trendState);
    ...
    if (in30.length >= 4) {
      ...
      if (early != null && late != null) memoryDeltaPct = Math.round((late - early) * 100);
    }
    ...
    const consistency =
      expected14 > 0 ? Math.min(100, Math.round((in14.length / expected14) * 100)) : 0;
    ...
    else if (completed.length) consistencyWord = 'building';
  ...
  /** @deprecated Never invent session history — kept as no-op so old callers are safe */
  async function ensureDemoHistory() {
    return;
  }

  return { build, normalizeSession, WEEK_TARGET, ensureDemoHistory };
})();
